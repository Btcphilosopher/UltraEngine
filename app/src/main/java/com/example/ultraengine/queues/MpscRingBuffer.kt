 ...
 * Multiple producer threads claim slots using atomic CAS sequences.
 * Single consumer thread drains without locking or contention.
 */
class MpscRingBuffer<E : Any>(requestedCapacity: Int = 1024) : RingBuffer<E> {
    ...
    // Multi-producer sequence counter
    private val producerHead = PaddedAtomicLong(0L)

    // Single-consumer sequence counter
    ...
    @HotPath("Lock-free CAS slot reservation")
    ...
    override fun poll(): E? {
        ...
        val element = buffer.get(index) ?: return null // Producer still writing
    ...
    @HotPath("Batch drain from single consumer")
